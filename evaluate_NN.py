...
for random_state in random_states:
    ...
    model_save_dir = os.path.join(
        PROJECT_ROOT, "saved_models", model_save_folder, f"random_state_{random_state}"
    )
    # Check if the output directory is empty
    os.makedirs(model_save_dir, exist_ok=True)
    if not len(os.listdir(model_save_dir)) == 0 and not prediction_only:
        overwrite = input(
            f"WARNING: The output directory {model_save_dir} is not empty, results might be overwritten. Do you want to continue? (y/N) "
        )
    ...
    for i, df in enumerate(df_list):
        ...
        if not h5py.is_hdf5(cache_file):
            ...
            if use_rdkit_descriptors:
                df_fp, descriptor_cols = calc_rdkit_descriptors(
                    df_fp, descriptors_list, missing_rdkit_desc
                )
            ...
            if len(selected_fp) > 0:
                X = torch.tensor(
                    np.concatenate(
                        [df_fp[fp].values.tolist() for fp in selected_fp.keys()], axis=1
                    ),
                    dtype=torch.float32,
                )
            if use_rdkit_descriptors:
                descriptors_X = torch.tensor(
                    df_fp[descriptor_cols].values.tolist(), dtype=torch.float32
                )
                X = torch.cat((X, descriptors_X), dim=1)
            y = torch.tensor(df_fp["Solubility"].values, dtype=torch.float32).reshape(
                -1, 1
            )
            ...
            if not prediction_only or not os.path.exists(
                os.path.join(model_save_dir, f"{solvents[i]}_model.pt")
            ):
                ...
                model_params_filename = os.path.join(
                    model_save_dir, f"{solvents[i]}_model_params.pkl"
                )
                ...
                scaler_filename = os.path.join(
                    model_save_dir, f"scaler_{solvents[i]}.pkl"
                )
                ...
                nn_model.load_state_dict(
                    torch.load(os.path.join(model_save_dir, f"{solvents[i]}_model.pt"))
                )
            nn_model.eval()
        ...
        assert (
            residuals.shape == true_values.shape
        ), f"Shape mismatch: {residuals.shape} vs. {true_values.shape}"
        ...
        plt.savefig(
            os.path.join(
                PLOTS_DIR,
                f"{input_type}DB_{solvents[i]}_true_vs_predicted_state_{random_state}.png",
            )
        )
        plt.close()
        ...
        plt.savefig(
            os.path.join(
                PLOTS_DIR,
                f"{input_type}DB_{solvents[i]}_residuals_vs_true_state_{random_state}.png",
            )
        )
        plt.close()
        ...
        plt.savefig(
            os.path.join(
                PLOTS_DIR,
                f"{input_type}DB_{solvents[i]}_residuals_distribution_state_{random_state}.png",
            )
        )
        plt.close()
        ...
        logger.info(
            f"Mean squared error for {solvents[i]}: {mse_mean} for the dummy model"
        )
        ...
        plt.savefig(
            os.path.join(
                PLOTS_DIR,
                f"{input_type}DB_{solvents[i]}_true_vs_mean_predicted_state_{random_state}.png",
            )
        )
        plt.close()
        ...
        plt.savefig(
            os.path.join(
                PLOTS_DIR,
                f"{input_type}DB_{solvents[i]}_residuals_distribution_mean_state_{random_state}.png",
            )
        )
        plt.close()

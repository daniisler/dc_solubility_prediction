...
# List of descriptors from a dataframe to use
...
param_grid = {
    ...
    "optimizer": [
        optim.RMSprop
    ],  # optim.SGD, optim.Adagrad, optim.Adamax, optim.AdamW, optim.RMSprop, optim.Adam, optim.Adadelta
    "loss_fn": [
        nn.functional.mse_loss
    ],  # nn.functional.mse_loss, nn.functional.smooth_l1_loss, nn.functional.l1_loss
    "activation_fn": [nn.Tanh],  # nn.ReLU, nn.Sigmoid, nn.Tanh, nn.LeakyReLU, nn.ELU
}

if param_grid and not prediction_only:
    logger.info(
        f"Performing hyperparameter optimization for the solvent(s) {solvents}..."
    )
    ...
    hyperparam_optimization(
        ...
        cached_input_dir=cached_input_dir,
        ...
        missing_rdkit_desc=missing_rdkit_desc,
        use_df_descriptors=use_df_descriptors,
        ...
        lr_min=lr_min,
        lr_mode=lr_mode,
        wandb_identifier=wandb_identifier,
...
if not all(
    os.path.exists(os.path.join(model_save_dir, f"weights_{solvent}.pth"))
    for solvent in solvents
):
    raise FileNotFoundError(
        f'Missing model weights in {model_save_dir} for solvent(s) {[solvent for solvent in solvents if not os.path.exists(os.path.join(model_save_dir, f"weights_{solvent}.pth"))]}!'
    )
# Same for params
if not all(
    os.path.exists(os.path.join(model_save_dir, f"params_{solvent}.pkl"))
    for solvent in solvents
):
    raise FileNotFoundError(
        f'Missing model parameters in {model_save_dir} for solvent(s) {[solvent for solvent in solvents if not os.path.exists(os.path.join(model_save_dir, f"params_{solvent}.pkl"))]}!'
    )
# Same for scalers
if not all(
    os.path.exists(os.path.join(model_save_dir, f"scaler_{solvent}.pkl"))
    for solvent in solvents
):
    raise FileNotFoundError(
        f'Missing scalers in {model_save_dir} for solvent(s) {[solvent for solvent in solvents if not os.path.exists(os.path.join(model_save_dir, f"scaler_{solvent}.pkl"))]}!'
    )
...
if do_prediction:  # TODO implement use_df_descriptors, descriptors_df_list
    ...
    for solvent in solvents:
        ...
        logger.info(
            f"The predicted solubility for the molecule with SMILES {smiles} in {solvent} at T={T} K is {solubility}."
        )

...
def hyperparam_optimization(
    ...
    if lr_mode not in ["min", "max", "abs"]:
        raise ValueError(
            f"Invalid lr_mode: {lr_mode}. Valid values are: min, max, abs."
        )
    if ES_mode != lr_mode:
        input(
            f"WARNING: ES_mode ({ES_mode}) and lr_mode ({lr_mode}) are not the same. This does not make sense! Press Enter to continue or Ctrl+C to exit."
        )
    if lr_threshold <= ES_min_delta and lr_patience >= ES_patience:
        input(
            f"WARNING: lr_threshold ({lr_threshold}) is smaller than or equal to ES_min_delta ({ES_min_delta}) and lr_patience ({lr_patience}) is larger than or equal to ES_patience ({ES_patience}). This will lead to early stopping before learning rate reduction. Press Enter to continue or Ctrl+C to exit."
        )
    ...
    if T:
        ...
        if main_df.empty:
            raise ValueError(
                f"No data found for temperature {T} K. Exiting hyperparameter optimization."
            )
    ...
    if any(df.empty for df in df_list):
        raise ValueError(
            f"No data found for {[solvent for solvent in solvents if df_list[solvents.index(solvent)].empty]} at T={T} K. Exiting hyperparameter optimization."
        )
    ...
    for i, df in enumerate(df_list):
        ...
        if os.path.exists(fingerprint_df_filename):
            ...
            for fp in selected_fp.keys():
                df_list_fp[i][fp] = df_list_fp[i][fp].apply(
    ...
    if use_rdkit_descriptors:
        ...
        for i, df in enumerate(df_list_fp):
            df_list_fp[i], descriptor_cols_list[i] = calc_rdkit_descriptors(
                df, descriptors_list, missing_rdkit_desc
            )
    ...
    for i, df in enumerate(df_list_fp):
        ...
        if len(selected_fp) > 0:
            X = torch.tensor(
                np.concatenate(
                    [df[fp].values.tolist() for fp in selected_fp.keys()], axis=1
                ),
                dtype=torch.float32,
            )
        if use_rdkit_descriptors:
            descriptors_X = torch.tensor(
                df[descriptor_cols_list[i]].values.tolist(), dtype=torch.float32
            )
            X = torch.cat((X, descriptors_X), dim=1)
        if use_df_descriptors:
            if all(col in df.columns for col in descriptors_df_list):
                descriptors_df_X = torch.tensor(
                    df[descriptors_df_list].values.tolist(), dtype=torch.float32
                )
                X = torch.cat((X, descriptors_df_X), dim=1)
                logger.info(
                    f"Added DataFrame columns {descriptors_df_list} to input data X"
                )
            else:
                missing_cols = [
        ...
        best_hyperparams, best_valid_score, best_model = grid_search_params(
            param_grid,
            train_dataset,
            valid_dataset,
            test_dataset,
            ...
            wandb_identifier=f"{wandb_identifier}_{solvents[i]}",
        ...
        with open(
            f'{output_paramoptim_path.replace(".json", "")}_{solvents[i]}.json',
            "w",
            encoding="utf-8",
        ) as f:
            ...
            logger.info(
                f"Hyperparameter optimization finished. Best hyperparameters: {best_hyperparams}, Best validation score: {best_valid_score}, logs saved to {output_paramoptim_path}"
            )
        # Save the best weights
        logger.info(
            f"Saving best weights to {model_save_dir}/weights_{solvents[i]}.pth"
        )
        with open(f"{model_save_dir}/params_{solvents[i]}.pkl", "wb") as f:
            # Save the best model hyperparameters
            logger.info(
                f"Saving best hyperparameters to {model_save_dir}/params_{solvents[i]}.pkl"
            )
        ...
        if best_model.state_dict():
            torch.save(
                ...
                os.path.join(model_save_dir, f"weights_{solvents[i]}.pth"),
            )
        else:
    ...
    for i, combination in enumerate(combinations):
        ...
        if restore_best_weights:
            checkpoint_callback = ModelCheckpoint(
                monitor="Validation loss", mode=ES_mode, save_top_k=1
            )
